...
input_dir <- "star_counts"
output_dir <- "star_combined_counts"
if (!dir.exists(output_dir)) dir.create(output_dir)

# List all count files
count_files <- list.files(input_dir, pattern = "_counts.csv$", full.names = TRUE)

# Extract configuration from filename
get_config <- function(filename) {
  str_match(basename(filename), ".*_(Q[0-9]+_L[0-9]+_G[0-9]+_X[0-9]+_J[0-9_]+)_counts.csv")[,2]
}
...
for (config in configs) {
  ...
  cat("✅ Combined STAR matrix saved to:", output_file, "\n")
}