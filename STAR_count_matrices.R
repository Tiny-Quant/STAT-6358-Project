...
if (length(args) < 2) stop("Usage: Rscript STAR_count_matrices.R <bam_file> <gtf_file>")
bam_file <- args[1]
gtf_file <- args[2]

output_dir <- "star_counts"
...
output_file <- file.path(output_dir, paste0(sample_id, "_", configuration, "_counts.csv"))
write_csv(count_df, output_file)
cat("✅ STAR gene count matrix saved to:", output_file, "\n")
